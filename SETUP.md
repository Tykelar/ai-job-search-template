...
If a command still uses `pdftotext -layout`, it must pass `-enc UTF-8` as well (Xpdf-based builds default to Latin-1 output). If **neither** extractor is available, `/apply` skips the mechanical check with a warning and falls back to a visual keyword review — everything else works normally.
...
```bash
# Bash / zsh / Git Bash
cd applications && lualatex -output-directory=<company>_<role> <company>_<role>/CV_<CVNameSlug>_<company>_<role>.tex && cd ..
cd applications && xelatex -output-directory=<company>_<role> <company>_<role>/CL_<CVNameSlug>_<company>_<role>.tex && cd ..
```

```powershell
# PowerShell
Set-Location applications; lualatex -output-directory=<company>_<role> <company>_<role>/CV_<CVNameSlug>_<company>_<role>.tex; Set-Location ..
Set-Location applications; xelatex -output-directory=<company>_<role> <company>_<role>/CL_<CVNameSlug>_<company>_<role>.tex; Set-Location ..
```
