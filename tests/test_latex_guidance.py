...
Three silent-failure modes live here, all found by the 2026-08-19 upstream
review (F9, F31, F34). Each one produces a clean compile and a green CI run
...
- F9: a bullet written as ``\\item [text]`` is parsed as the item's
...
- F34: `pdftotext` without `-enc UTF-8` emits Latin-1 on Xpdf builds,
  so a correct non-ASCII CV fails the documented "no replacement
  characters" check and the agent is sent to "fix" a healthy document.

Fork adaptation (upstream refs `b2545d512`, `73d52e099`): the example
documents live under `applications/` here, not `cv/` + `cover_letters/`,
and the CV compiles with lualatex (fontspec), so upstream's `\\ifpdftex`
T1-fontenc guard does not apply — see the skipped class at the bottom.
"""
import re
import unittest
...
# Fork paths: the curated master and its cover-letter sibling live in applications/.
EXAMPLE_CV = REPO / "applications" / "main_example.tex"
EXAMPLE_COVER = REPO / "applications" / "cover_example.tex"
...
class TestBulletBracketTrap(unittest.TestCase):
    ...
    def assert_no_unbraced_bracket_items(self, path):
        ...
        self.assertEqual(
            offending,
            [],
            "an \\item followed by [ is parsed as an optional label and the "
...
@unittest.skip(
    "fork CV/cover compile with lualatex (fontspec); there is no pdflatex "
    "fallback, so upstream's \\ifpdftex T1-fontenc guard (ref 73d52e099) does "
    "not apply. Re-enable if the fork ever ships a pdflatex-compilable template."
)
