...
class VerifyPdfTests(unittest.TestCase):
    ...
    @patch("tools.verify_pdf._extract_pypdf", return_value=None)
    @patch("tools.verify_pdf.run_tool")
    def test_text_only_check_survives_missing_pdfinfo(self, mock_run_tool, _pypdf):
        # Git for Windows puts an xpdf pdftotext on PATH with no pdfinfo sibling;
        # a --contains/--dump-text check must not die because --pages was not asked for.
        def side_effect(command):
            if command[0] == "pdfinfo":
                raise VerificationError("required command 'pdfinfo' was not found.")
            return "Tech stack: Python\n"

        mock_run_tool.side_effect = side_effect
        extractor, text, pages = verify_pdf(self.pdf, required_text=("Tech stack:",))
        self.assertEqual(extractor, "pdftotext")
        self.assertEqual(text, "Tech stack: Python\n")
        self.assertIsNone(pages)

    @patch("tools.verify_pdf._extract_pypdf", return_value=None)
    @patch("tools.verify_pdf.run_tool")
    def test_pages_check_still_fails_loudly_when_pdfinfo_missing(self, mock_run_tool, _pypdf):
        def side_effect(command):
            if command[0] == "pdfinfo":
                raise VerificationError("required command 'pdfinfo' was not found.")
        ...
        mock_run_tool.side_effect = side_effect
        with self.assertRaisesRegex(VerificationError, "page count was requested"):
            verify_pdf(self.pdf, expected_pages=2)
