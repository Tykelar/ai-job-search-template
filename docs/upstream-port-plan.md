...
| 4 | PDF tooling & ATS verification stack | ☑ done | merge `PENDING` into `personal` (part branch `port/part-04-pdf-tooling` @ `2a80e9e`) | Method A take of `tools/verify_pdf.py`, `tests/test_verify_pdf.py`, `tests/test_latex_guidance.py`, + new `tools/verify_layout.py`/`tests/test_verify_layout.py`. 51 Part-4 tests green; full suite 313 (skipped=3). Deviations: `verify_layout` docstring escape; `verify_pdf` tolerates a missing `pdfinfo` on text-only checks (`--pages` still fails); `test_gate_configuration` also guards `verify_pdf.py`. `tests/test_tools_utf8_output.py` deferred to Part 7/13 per the Part 3 handoff. |
...
`tests/test_gate_configuration.py` also asserts `verify_pdf.py`. The two tool files
are therefore no longer byte-identical to upstream - reconcile by hand if upstream
changes them. `tests/test_tools_utf8_output.py` is deferred to Part 7/13.
