...
[![CI](https://github.com/Tykelar/ai-job-search-template/actions/workflows/ci.yml/badge.svg)](https://github.com/Tykelar/ai-job-search-template/actions/workflows/ci.yml)
...
```
/setup          /scrape           /rank              /apply
  |                |                 |                  |
  v                v                 v                  v
Fill in        Search job        Gate + score       Draft CV + Cover Letter
your profile   portals           every posting      (LaTeX, tailored)
  |                |                 |                  |
  v                v                 v                  v
Profile        Collect &         Ranked shortlist   Reviewer agent critiques
files ready    deduplicate       + honest gaps      -> Revise -> Final output
               (no gates,             |
                no drafting)          v
                                 Pick a number
```
...
- Optional: `pip install pypdf` for `/apply`'s ATS parseability check (BSD; no Poppler required). Poppler `pdftotext` remains a fallback (macOS: `brew install poppler`, Debian/Ubuntu: `apt install poppler-utils`, Windows: `choco install poppler`); when a fallback runs it must pass `-enc UTF-8`. If both are missing, the check degrades to a visual keyword review.
...
```bash
gh repo fork Tykelar/ai-job-search-template --clone
...
This searches multiple job portals for positions matching your profile, deduplicates results, and presents them sorted by fit. `/scrape` collects only — it applies no deal-breaker gates and drafts nothing — so the next step is `/rank`, which fetches each posting, runs the four hard gates from `config/gates.md`, scores whatever clears them, and returns a ranked shortlist. You can also hand one posting straight to `/apply`; it will tell you that job is ungated, since gates run only in `/rank`.
...
`/setup`, `/scrape`, `/rank`, and `/apply` form the core workflow - `/scrape` collects, `/rank` gates and scores, `/apply` builds. The ten commands below cover the rest of the pipeline, with `/rank` described among them since it is where the deal-breaker gates live:
...
- **`/rank`** bridges `/scrape` and `/apply`, and is **the one place the hard gates run**: parallel agents fetch each posting, apply the four gates from `config/gates.md` (work authorization, language, experience ceiling, authorized countries), then score the survivors on four weighted dimensions and return a ranked shortlist with honest per-job strengths and gaps. A gate failure is never scored — it leaves the pool with the requirement quoted from the posting. Deadlines get urgency flags, dead postings get marked expired. Pick a number and it hands off to `/apply`, which carries the verdict forward rather than re-deriving it.
...
├── config/
│   └── gates.md                       # The four hard gates: work authorization, languages,
│                                      #   experience ceiling, authorized countries (single
│                                      #   source of truth; /rank Step 2a is where they run)
├── .claude/
│   ├── commands/
...
│   │   │   ├── 04-job-evaluation.md   # Gate mechanisms + scoring framework (values in config/gates.md)
...
7. **ATS-check the CV**: extract the PDF's text layer (`tools/verify_pdf.py`, pypdf or optional Poppler) and verify it the way an ATS parser sees it — contact details present as literal text, no garbled glyphs, sane reading order — then score the posting's keyword coverage against the extraction. Keywords the profile genuinely supports get covered by surfacing the master lines that carry them; genuine gaps stay visible, never stuffed.
...
- **ATS verification on the PDF text layer.** An ATS reads the PDF's embedded text, not the rendered page — and LaTeX can silently produce PDFs whose text extracts as garbage (icon glyphs where the email should be, interleaved lines from multi-column layouts). `/apply` extracts the compiled CV's text layer with `tools/verify_pdf.py` (pypdf first, Poppler `pdftotext -layout -enc UTF-8` fallback) and verifies contact details, reading order, and the posting's keyword coverage against what a parser actually sees. `tools/verify_layout.py` measures the page geometry first, so a hole left by an entry that could not fit is caught before the human eye has to notice it. Honesty rule enforced: a keyword the profile doesn't support is acknowledged as a gap, never stuffed in.
