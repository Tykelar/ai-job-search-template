...
def _extract_pdftotext(pdf_path):
    ...
    # Poppler ships pdfinfo alongside pdftotext, but that pairing is not
    # guaranteed: Git for Windows puts an xpdf pdftotext on PATH with no pdfinfo
    # sibling. A text-only check (--contains/--dump-text, no --pages) must not
    # die because the page counter is missing - the caller that needs pages
    # re-checks for None and fails then.
    try:
...
def extract_text_layer(pdf_path):
    ...
    `pages` is None when only pdftotext is available and pdfinfo is not; the
    pypdf path always knows its page count.
    """
...
def verify_pdf(pdf_path, expected_pages=None, min_chars=1, required_text=(), dump_text=None):
    ...
    if expected_pages is not None:
        if actual_pages is None:
            raise VerificationError(
                "a page count was requested but pdfinfo is unavailable; install "
                "pypdf (`pip install pypdf`) or poppler-utils"
            )
...
def main(argv=None):
    ...
    try:
        ...
        return 1
    page_label = pages if pages is not None else "unknown"
    print(f"Verified {args.pdf} (extractor: {extractor}, pages: {page_label})")
    return 0
