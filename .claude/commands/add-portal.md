...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes:
   ...
   - Optionally add a WebSearch `site:` fallback query for that board to your local, gitignored `.claude/skills/job-scraper/search-queries.md` (follow the `[YOUR_JOB_BOARD]`-style placeholder shape documented in the tracked `search-queries.template.md`) so the fallback path still covers the board if the CLI is unavailable.
