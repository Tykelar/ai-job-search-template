...
**Output file:** `applications/<NN>_<company>_<role>/CL_<CVNameSlug>_<company>_<role>.tex`
...
Expected output: `Output written on CL_<CVNameSlug>_<company>_<role>.pdf (1 page, ...)`. Any page count other than 1 is a failure that must be fixed before presenting to the user.
...
1. Run `cd applications && xelatex -interaction=nonstopmode -output-directory=<NN>_<company>_<role> <NN>_<company>_<role>/CL_<CVNameSlug>_<company>_<role>.tex`
...
- A bullet whose text begins with a literal `[` must be braced: `\item {[text]}`.
  Unbraced, LaTeX parses the bracketed text as the item's optional label and renders it
  off the left page edge, missing from the PDF text layer entirely
...
- File naming is already handled by the output convention: `CV_<CVNameSlug>_<company>_<role>.pdf` and `CL_<CVNameSlug>_<company>_<role>.pdf`
