...
<!-- SETUP: Populated by running /setup from your profile. These rules override the
     generic guidance above for your documents only. If you maintain a USI corpus,
     /sync-usi can regenerate this section from it; otherwise edit it directly.
     Internal positioning guidance: informs how documents are written; never quote
     this section itself in outgoing documents. -->

## Headline rules (hard)
- [YOUR_DEFAULT_HEADLINE_AND_RULES] <!-- e.g. default headline, and when a "Graduate" label is allowed -->

## Framing rules (hard)
- [TITLE_OR_FRAMING_YOU_NEVER_WANT]
- [HOW_THAT_ROLE_SHOULD_BE_FRAMED_INSTEAD]

## Primary target framings
[YOUR_TOP_PRIORITY_ROLE_TYPES - the framings to default to when a role is ambiguous.]

## Voice
- [VOICE_NOTES_SPECIFIC_TO_YOU]

## Facts discipline
- Every fact must trace to `01-candidate-profile.md` (and `applications/main_example.tex` / `applications/master_cv.md` for CV content). If a fact the role needs is missing, add it to the profile first - never improvise it into a document.
